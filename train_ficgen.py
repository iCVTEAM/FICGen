...
from transformers import CLIPTextModel, CLIPTextModelWithProjection, CLIPTokenizer, CLIPVisionModel, CLIPVisionModelWithProjection, AutoProcessor
...
from src.utils import get_similar_examplers, get_classnames
...
def parse_args():
    ...
    parser.add_argument("--img_patch_path",type=str,default=None)
    parser.add_argument("--ref_resolution", type=int, default=256)
    parser.add_argument(
        "--image_encoder_path",
        type=str,
        # default='CLIP-ViT-H-14',
        default='/cpfs/user/wenzhuangwang/aerogen/ckpt/clip/clip-vit-large-patch14',
    )
...
def main():
    ...
    # `from_pretrained` So CLIPTextModelWithProjection and AutoencoderKL will not enjoy the parameter sharding
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        text_projector = CLIPTextModelWithProjection.from_pretrained(args.image_encoder_path)
        image_processor = AutoProcessor.from_pretrained(args.image_encoder_path)
    ...
    image_proj_model = SerialSampler(
        dim=1280,
        depth=4,
        dim_head=64,
        num_queries=[16, 8, 8],
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            ...
            for i in range(len(models)):
                ...
                state_dict = load_file(os.path.join(input_dir, 'unet/diffusion_pytorch_model.safetensors'))
    ...
    params_count = 0
    for name, params in custom_layers.named_parameters():
        params_count += params.numel()
    for name, params in image_proj_model.named_parameters():
        params_count += params.numel()
        
    print("Params Count:{:.2f}M.".format(params_count/10**6))
    ...
    def _encode_prompt(
            ...
            if prompt_embeds_none_flag:
              ...
              #          "The following part of your input was truncated because CLIP can only handle sequences up to"
               ...
               #     )
                ...
                # embeds_pooler = prompt_embeds.last_hidden_state[:, 0, :]
                embeds_pooler = prompt_embeds.pooler_output
                prompt_embeds = prompt_embeds.last_hidden_state
                
                text_embeds = text_projector(text_input_ids.to(device)).text_embeds

            prompt_embeds = prompt_embeds.to(dtype=text_encoder.dtype, device=device)
            embeds_pooler = embeds_pooler.to(dtype=text_encoder.dtype, device=device)
            text_embeds = text_embeds.to(dtype=text_encoder.dtype, device=device)
        ...
        return prompt_embeds, embeds_pooler[:, None, :], text_embeds
    ...
    instance_transforms = transforms.Compose(
        [
            transforms.Resize([args.ref_resolution, args.ref_resolution], interpolation=transforms.InterpolationMode.BILINEAR),
            transforms.ToTensor(),
            transforms.Normalize(
                [0.5], [0.5]
            ),
    ...
    dict_of_images = {}
    list_of_name, data_emb_dict = get_classnames(args.train_data_dir)
        ...
        for index, (caption, bboxes) in enumerate(zip(examples["captions"], examples["bboxes"])):
            ...
            for name, bbox in zip(categories, bboxes):
                if name == '':
                    ...
                    value = (bbox[2] - bbox[0]) / (bbox[3] - bbox[1] + 1e-10)    # foreground aspect ratio based on HBB
    ...
    text_projector.to(accelerator.device, dtype=weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                cond_prompt_embeds, embeds_pooler, text_embeds = _encode_prompt(prompts=batch["prompts"], device=latents.device,num_images_per_prompt=1, do_classifier_free_guidance=False)
                ...
                height = 512 
                width = 512 
                ref_imgs = batch["instances"].view(bsz * len(obboxes[0]), 3, 256, 256)
                with torch.no_grad():
